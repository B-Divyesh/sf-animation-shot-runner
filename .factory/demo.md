...
The browser demo stores `demo:animation-shot-runner:opened` under its separate demo namespace. During an internal page change, the site also keeps one short-lived route marker in session storage to restore scroll and focus; it is removed on arrival and contains no sample data. The persistent banner says “Demo — sample data, nothing is saved” and provides **Reset demo** and **Start for real**. Reset recreates only that demo key. Start for real removes every `demo:animation-shot-runner:*` key, preserves non-demo storage, and returns to `/`.
