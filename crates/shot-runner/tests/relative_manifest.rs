...
fn direct_command_expands_every_placeholder_without_shell_interpretation() {
    ...
    #[cfg(unix)] { use std::os::unix::fs::PermissionsExt; fs::set_permissions(&script, fs::Permissions::from_mode(0o755)).expect("make script executable"); }
    DynamicImage::ImageRgba8(ImageBuffer::from_pixel(80, 45, Rgba([20, 30, 40, 255])))
        .save(dir.path().join("fixture.png")).expect("image fixture");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_shot-runner"))
        .current_dir(dir.path())
        .args(["run", "shots.json", "--allow-command", script_text.as_ref(), "--yes"])
        .output().expect("run direct argv fixture");
    ...
    assert!(!dir.path().join("should-not-exist").exists(), "runner must not invoke a shell");
}

#[test]
fn documented_relative_paths_cache_and_exit_codes_hold() {
    let dir = tempdir().expect("temporary consumer project");
    let project = dir.path().join("project"); fs::create_dir(&project).expect("project");
